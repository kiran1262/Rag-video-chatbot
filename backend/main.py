...
async def process_videos(request: ProcessVideosRequest):
    ...
    try:
        ...
        # Initialize RAG chain for this session, seeded with engagement metrics
        chain = RAGChain()
        chain.set_stats(result.get("videos", {}))
        rag_chains[request.session_id] = chain
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # Sessions live in memory; a missing one means it was never processed
        # (or the server restarted). Don't fabricate from another session's data.
        if request.session_id not in rag_chains:
            return {
                "response": "No analyzed videos for this session. Process the two video URLs first, then ask. (Sessions are in-memory — re-process if the backend restarted.)",
                "sources": [],
            }
...
async def chat_stream(session_id: str, query: str):
    ...
    try:
        ...
        if session_id not in rag_chains:
            def generate():
                msg = "No analyzed videos for this session. Process the two video URLs first, then ask."
        ...
        def generate():
            for chunk in rag_chain.stream_response(query, session_id):
